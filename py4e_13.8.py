...
# Actual data: http://py4e-data.dr-chuck.net/comments_28264.json (Sum ends with 22)
...
while True:
    ...
    for i in range(len(info['comments'])):
        # Finn item 'comments' (liste) i info (dict), finn liste-item med index i, og hent ut 'count' item:
        number = info['comments'][i]['count']
        sum += int(number)
        count += 1
